...
def combined_data(Towers=['K67', 'K77', 'K83']):
    ...
    selected_columns = ['Year_LBAMIP', 'DoY_LBAMIP', 'Hour_LBAMIP', 'NEEnogap_5day_sco2_ust', 'GEP_5day_sco2_ust']
    ...
    mds = ee.ImageCollection("MODIS/061/MOD13A1")
    mds = mds.select('NDVI', 'EVI').filterDate(i_date, f_date)
    ...
    def add_date_column_mds(df):
        df['date'] = df['id'].str.extract(r'(\d{4}_\d{2}_\d{2})', expand=False)
        df['date'] = pd.to_datetime(df['date'].str.replace('_', '-'), format='%Y-%m-%d')
    ...
    for tower in Towers:
        ...
        ec_df.rename(columns={'Hour_LBAMIP': 'Hour', 'NEEnogap_5day_sco2_ust': 'NEE', 'GEP_5day_sco2_ust': 'GEP'}, inplace=True)
        ...
        mds_poi = mds.getRegion(poi, scale).getInfo()
        mds_df = pd.DataFrame(mds_poi[1:], columns=mds_poi[0])
        mds_df = add_date_column_mds(mds_df)
        
        df = pd.merge(ec_df, sat_df, on='date', how='inner')
        df_1 = pd.merge(ec_df, mds_df, on='date', how='inner')
        ...
        data_dict_LST[tower] = df
        data_dict_MDS[tower] = df_1
    
    return data_dict_LST, data_dict_MDS